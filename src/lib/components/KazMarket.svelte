...
<div class="flex flex-col gap-5">
	...
	<div class="flex flex-wrap items-center justify-between gap-3">
		...
		<div class="flex items-center gap-2">
			...
			<button class="flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-sm font-bold text-white transition hover:brightness-110" on:click={() => (showPicker = !showPicker)}>
	...
	<!-- Invest picker -->
	{#if showPicker}
		<div class="flex flex-col gap-2 rounded-box border border-base-content/10 bg-base-200 p-3">
			...
			<div class="grid max-h-64 grid-cols-1 gap-2 overflow-y-auto sm:grid-cols-2">
				{#each pickerResults as a (a.assetId)}
					<button class="flex items-center gap-2 rounded-xl border border-base-content/10 bg-base-100 p-2 text-left transition hover:border-primary" on:click={() => { showPicker = false; openTrade({ ...a, price: 10 }, 'buy'); }}>
	...
	<div class="flex flex-col gap-2">
		...
		{#if loading}
			{#each Array(5) as _}<div class="h-16 animate-pulse rounded-box bg-base-200"></div>{/each}
		...
		{:else}
			{#each assets as a (a.id)}
				<div class="flex items-center gap-3 rounded-box border border-base-content/10 bg-base-200 p-3">
					...
					<button class="flex-none rounded-full bg-primary px-4 py-2 text-sm font-bold text-white transition hover:brightness-110" on:click={() => openTrade(a, 'buy')}>Invest</button>
				</div>
			{/each}
		{/if}
	</div>
...
<!-- Trade modal -->
{#if trade}
	<div class="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 p-4" on:click={() => (trade = null)} on:keydown role="button" tabindex="0">
		<div class="w-full max-w-sm rounded-box border border-base-content/10 bg-base-100 p-6 shadow-md" on:click|stopPropagation on:keydown|stopPropagation role="dialog" aria-modal="true" tabindex="-1">
			...
			<div class="flex gap-2">
				...
				<button class="flex-1 rounded-xl bg-primary py-3 font-bold text-white transition hover:brightness-110 disabled:opacity-50" on:click={confirmTrade} disabled={trading || !tradeAmount}>{trading ? '…' : trade.action === 'buy' ? 'Invest' : 'Sell'}</button>
			</div>
		</div>
	</div>
{/if}
