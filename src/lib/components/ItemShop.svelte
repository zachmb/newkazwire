<script lang="ts">
	...
	async function buy(item: any) {
		...
		buyingId = item.id;
		try {
			const r = await fetch('/api/shop/buy', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ uid, itemId: item.id }) });
			const j = await r.json();
			if (!r.ok || !j.success) { if (typeof j.balance === 'number') coins = j.balance; throw new Error(j.error || 'Purchase failed'); }
			coins = j.balance;
			item.soldCount = (item.soldCount || 0) + 1;
			items = items;
			showToast(`Bought "${item.title}"! ${coins} coins left`);
		} catch (e: any) { showToast(e.message || 'Purchase failed', 'error'); } finally { buyingId = ''; }
	}
	...
	async function submitListing() {
		...
		listing = true;
		try {
			const r = await fetch('/api/shop', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ uid, sellerName: playerName, title: fTitle, description: fDesc, price: fPrice, icon: fIcon }) });
			const j = await r.json();
			if (!r.ok || !j.success) throw new Error(j.error || 'Could not list item');
			items = [j.item, ...items];
			showList = false; fTitle = ''; fDesc = ''; fPrice = null; fIcon = 'mdi:package-variant';
			showToast('Item listed!');
		} catch (e: any) { showToast(e.message || 'Could not list item', 'error'); } finally { listing = false; }
	}
...
<div class="flex flex-col gap-4">
	<div class="flex flex-wrap items-center justify-between gap-3">
		...
		<div class="flex items-center gap-2">
			<div class="flex items-center gap-2 rounded-full bg-warning/15 px-4 py-2 font-black text-warning">
	...
	{#if showList}
		<div class="flex flex-col gap-3 rounded-2xl bg-base-200 p-5 ring-1 ring-base-300">
			...
			<div class="flex items-center gap-3">
				...
				<button class="rounded-full bg-primary px-5 py-2 text-sm font-bold text-white transition hover:brightness-110 disabled:opacity-50" on:click={submitListing} disabled={listing}>{listing ? 'Listing…' : 'List item'}</button>
			</div>
		</div>
	{/if}
			...
			{#each items as item (item.id)}
				<div class="flex flex-col gap-2 rounded-2xl bg-base-200 p-4 ring-1 ring-base-300 transition hover:ring-primary">
					...
					<div class="mt-auto flex items-center justify-between gap-1 pt-1">
						...
						<button class="rounded-full bg-primary px-3 py-1.5 text-xs font-bold text-white transition hover:brightness-110 disabled:opacity-50" on:click={() => buy(item)} disabled={buyingId === item.id || item.sellerUid === uid}>
