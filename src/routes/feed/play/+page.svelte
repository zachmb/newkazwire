<!--
  /feed/play — TikTok-style vertical swipe feed for browsing games one-per-screen.
  (Moved here from /feed, which is now the Twitter/X-style POSTS feed. This is the
  "Play" tab reachable from the /feed tab bar.)
...
<svelte:head>
	<title>Play Feed · Kazwire</title>
	<meta name="description" content="Swipe through Kazwire games one at a time — AI community creations and the full library." />
</svelte:head>

<svelte:window onkeydown={onKey} />

<!-- Back-to-posts tab pill (mirrors the /feed tab bar so Play is escapable) -->
<a
	href="/feed"
	class="fixed left-4 top-4 z-30 inline-flex items-center gap-1.5 rounded-full bg-black/50 px-3 py-1.5 text-xs font-bold text-white backdrop-blur transition hover:bg-black/70 pt-[env(safe-area-inset-top)]"
>
