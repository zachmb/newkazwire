...
<div class="kz-page">
	<div class="overflow-hidden rounded-box border border-base-content/10 bg-base-200 shadow-sm">
				...
				<div class="mt-10 space-y-6">
					{#each channels as c}
						<a href={c.href} class="flex items-center gap-4 transition hover:opacity-90" target={c.href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer">
							<div class="grid h-12 w-12 place-items-center rounded-xl bg-white/10 text-primary">
			...
			<div class="bg-base-100 p-8 md:p-12">
				<form on:submit={send} class="flex flex-col gap-5">
					<div>
						...
						<input id="name" bind:value={name} type="text" placeholder="Alex Player" class="w-full rounded-xl border border-base-content/10 bg-base-200 p-4 font-semibold text-base-content outline-none transition focus:border-primary focus:bg-base-100" />
					</div>
					<div>
						<label for="email" class="mb-2 block text-xs font-black uppercase tracking-widest text-base-content/50">Email</label>
						<input id="email" bind:value={email} type="email" placeholder="you@example.com" class="w-full rounded-xl border border-base-content/10 bg-base-200 p-4 font-semibold text-base-content outline-none transition focus:border-primary focus:bg-base-100" />
					</div>
					<div>
						<label for="message" class="mb-2 block text-xs font-black uppercase tracking-widest text-base-content/50">Message</label>
						<textarea id="message" bind:value={message} rows="5" placeholder="What's on your mind?" class="w-full rounded-xl border border-base-content/10 bg-base-200 p-4 font-semibold text-base-content outline-none transition focus:border-primary focus:bg-base-100"></textarea>
					</div>
					<button type="submit" class="mt-2 flex items-center justify-center gap-2 rounded-xl bg-primary py-4 text-lg font-black text-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
